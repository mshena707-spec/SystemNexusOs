...
export class OpenAIAdapter implements IAIProvider {
  ...
  async generateChat(messages: ChatMessage[], systemPrompt?: string): Promise<string> {
    if (!this.apiKey) { console.warn('[OpenAIAdapter] OPENAI_API_KEY not set'); return ''; }
  ...
  async *generateChatStream(messages: ChatMessage[], systemPrompt?: string): AsyncGenerator<string> {
    ...
    if (!res.ok || !res.body) throw new Error(`OpenAI stream error: ${await res.text()}`);
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    if (!res.ok) return new Array(1536).fill(0);
